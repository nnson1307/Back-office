var adminMenu = {
    ...
    showPopupAdminGroup: function () {
        ...
        $.ajax({
            url: laroute.route('user.admin-menu.get-list-group'),
            method: 'POST',
            data: {
    ...
    addGroup: function () {
        ...
        $.ajax({
            url: laroute.route('user.admin-menu.add-collection-list'),
            method: 'POST',
            data: {
    ...
    removeMenu: function (menu_id) {
        $.getJSON(laroute.route('user.validation'), function (json) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: laroute.route('user.admin-menu.destroy'),
                        method: 'POST',
                        data: {
                            menu_id: menu_id
                        },
    ...
    save: function (is_quit = 0) {
        ...
        $.getJSON(laroute.route('user.validation'), function (json) {
            ...
            if (form.valid()) {
                var url = ($('#menu_id').length) ? laroute.route('user.admin-menu.update') : laroute.route('user.admin-menu.store')
                $.ajax({
                    url: url,
                    method: 'POST',
                    dataType: 'JSON',
                            ...
                            swal.fire(res.message, "", "success").then(function () {
                                if (is_quit === 0) {
                                    if ($('#menu_id').length) {
                                        ...
                                        window.location.href = laroute.route('user.admin-menu.create');
                                    }
                                } else {
                                    window.location.href = laroute.route('user.admin-menu.index');
                                }
                            });
                        } else {
