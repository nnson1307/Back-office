...
var Add = {
    submitAdd: function (action) {
        ...
        $('#form-adds').validate({
            ...
            submitHandler: function () {
                ...
                $.ajax({
                    url: laroute.route('user.my-store.store'),
                    method: "POST",
                    dataType: "JSON",
                    data: {
    ...
    createPasswordNew: function () {
        ...
        if (autoGeneratePassword == 0) {
            $('#form-reset-password').validate({
                ...
                submitHandler: function () {
                    $.ajax({
                        url: laroute.route('user.my-store.update-password'),
                        method: "POST",
                        dataType: "JSON",
                        data: {
    ...
    selectGroupChild: function (group_id = 0) {
        ...
        $.ajax({
            url: laroute.route('user.my-store.get-list-child'),
            method: 'POST',
            data: {
    ...
    addGroupChild: function () {
        ...
        $.ajax({
            url: laroute.route('user.my-store.add-collection-list-child'),
            method: 'POST',
            data: {
    ...
    change_status: function (id, obj) {
        ...
        $.ajax({
            url: laroute.route('user.my-store.change-status'),
            dataType:'JSON',
            method:'POST',
            data:{
                id:id,
...
var list = {
    action: function (o) {
        ...
        if (t.val() == 'reset_password') {
            $.ajax({
                url: laroute.route('user.my-store.show-reset-password'),
                method: 'POST',
                data: {
                    user_id: userId
                },
            ...
            javascript:location.href = laroute.route('user.my-store.edit',{id:userId});
            ...
            swal.fire({
                ...
                if (result.value) {
                    javascript:location.href = laroute.route('user.my-store.destroy',{id:userId});
                }
            });
        }
    },
    remove: function (id) {
        swal.fire({
            ...
            if (result.value) {
                javascript:location.href = laroute.route('user.my-store.destroy', {id: id});
            }
        });
    },
    ...
    sort: function (o, col) {
        ...
        switch (col) {
            case 'email':
                $("#sort_email").val(sort);
                $("#sort_full_name").val(null);
                $("#sort_is_active").val(null);
                break;
            case 'full_name':
                $("#sort_email").val(null);
                $("#sort_full_name").val(sort);
                $("#sort_is_active").val(null);
                break;
            case 'is_actived':
                $("#sort_email").val(null);
                $("#sort_full_name").val(null);
                $("#sort_is_active").val(sort);
                break;
        }
        list.filter();
    }
