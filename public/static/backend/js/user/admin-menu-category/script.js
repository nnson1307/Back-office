var adminMenuCategory = {
    ...
    save: function (is_quit = 0) {
        ...
        $.getJSON(laroute.route('user.validation'), function (json) {
            ...
            if (form.valid()) {
                var url = ($('#menu_category_id').length) ? laroute.route('user.admin-menu-category.update') : laroute.route('user.admin-menu-category.store');
                $.ajax({
                    url: url,
                    method: 'POST',
                    dataType: 'JSON',
                        ...
                        if (!res.error) {
                            if (is_quit === 0) {
                                if ($('#menu_category_id').length) {
                                    ...
                                    swal.fire(res.message, "", "success").then(function () {
                                        window.location.href = laroute.route('user.admin-menu-category.create');
                                    });
                                }
                            } else {
                                swal.fire(res.message, "", "success").then(function () {
                                    window.location.href = laroute.route('user.admin-menu-category.index');
                                });
                            }
                        } else {
    ...
    remove: function (id) {
        $.getJSON(laroute.route('user.validation'), function (json) {
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: laroute.route('user.admin-menu-category.destroy'),
                        method: 'POST',
                        data: {
                            menu_category_id: id
                        },
