        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    <th id="th_menu_category_name">
                        <p class="pn-pointer" onclick="adminMenuCategory.sort(this, 'menu_category_name')"
                           data-sort="{{ ($filter['sort_admin_menu_category$menu_category_name'] == 'asc') ? 'desc' : 'asc' }}">
                            @lang('user::admin-menu-category.table.COLUMN_ADMIN_MENU_CATEGORY_NAME')
                    ...
                    <th id="th_menu_category_position">
                        <p class="pn-pointer" onclick="adminMenuCategory.sort(this, 'menu_category_position')"
                           data-sort="{{ ($filter['sort_admin_menu_category$menu_category_position'] == 'asc') ? 'desc' : 'asc' }}">
                            @lang('user::admin-menu-category.table.COLUMN_ADMIN_MENU_CATEGORY_POSITION')
                    ...
                    <th><p>@lang('user::admin-menu-category.index.ACTION')</p></th>
                </tr>
            </thead>
            <tbody>
                @if (isset($list) && $list->count() > 0)
                    @foreach ($list as $item)
                        <tr>
                            <td>
                                <a href="{{ route('user.admin-menu-category.show', ['id' => $item['menu_category_id']]) }}"
                                ...
                                <div class="kt-portlet__head-toolbar">
                                    <div class="btn-group" role="group">
                                        <button id="btnGroupVerticalDrop1" type="button" class="btn btn-secondary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                            @lang('user::admin-menu-category.table.COLUMN_ACTION')
                                        </button>
                                        <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop1">
                                            @include('helpers.button', ['button' => [
                                                    'route' => 'user.admin-menu-category.edit',
                                                     'html' => '<a href="'.route('user.admin-menu-category.edit', ['id' => $item['menu_category_id']]).'" class="dropdown-item">'
                                                     .'<i class="la la-edit"></i>'
                                                     .'<span class="kt-nav__link-text kt-margin-l-5">'.__('user::admin-menu-category.input.BUTTON_EDIT').'</span>'.
                                                '</a>'
                                            ]])
                                            @include('helpers.button', ['button' => [
                ...
                @else
                    <tr>
                        <td colspan="3" class="text-center">@lang('user::admin-menu-category.table.DATA_NULL')</td>
                    </tr>
                @endif
            </tbody>
        </table>
