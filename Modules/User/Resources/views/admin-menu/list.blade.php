    ...
    <form id="form-filter" action="{{route('user.admin-menu.index')}}">
        <table class="table table-striped">
            ...
            <tr>
                <th id="th_menu_name">
                    <p class="pn-pointer" onclick="adminMenu.sort(this, 'menu_name')"
                       data-sort="{{ ($filter['sort_admin_menu$menu_name'] == 'asc') ? 'desc' : 'asc' }}">
                        @lang('user::admin-menu.table.ADMIN_MENU_NAME')
                ...
                <th id="th_menu_category_name">
                    <p class="pn-pointer" onclick="adminMenu.sort(this, 'menu_category_name')"
                       data-sort="{{ ($filter['sort_admncat$menu_category_name'] == 'asc') ? 'desc' : 'asc' }}">
                        @lang('user::admin-menu.table.ADMIN_MENU_CATEGORY_NAME')
                ...
                <th id="th_menu_position">
                    <p class="pn-pointer" onclick="adminMenu.sort(this, 'menu_position')"
                       data-sort="{{ ($filter['sort_admin_menu$menu_position'] == 'asc') ? 'desc' : 'asc' }}">
                        @lang('user::admin-menu.table.ADMIN_MENU_POSITION')
                ...
                <th><p>@lang('user::admin-menu.index.ACTION')</p></th>
            </tr>
            </thead>
            <tbody>
            @if (isset($list) && $list->count() > 0)
                @foreach ($list as $item)
                    <tr>
                        <td>
                            <a href="{{ route('user.admin-menu.show', ['id' => $item['menu_id']]) }}"
                            ...
                            <div class="kt-portlet__head-toolbar">
                                <div class="btn-group" role="group">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop1">
                                        <a href="{{ route('user.admin-menu.show', ['id' => $item->menu_id]) }}" class="dropdown-item">
                                            <i class="la la-search"></i>
                                            <span class="kt-nav__link-text kt-margin-l-5">
                                            @lang('user::admin-menu.index.TABLE_VIEW')
                                        </span>
                                        </a>
                                        @include('helpers.button', ['button' => [
                                                    'route' => 'user.admin-menu.edit',
                                                     'html' => '<a href="'.route('user.admin-menu.edit', ['id' => $item->menu_id]).'" class="dropdown-item">'
                                        ...
                                        @include('helpers.button', ['button' => [
                                                   'route' => 'user.admin-menu.destroy',
                                                    ...
                                                    .'<span class="kt-nav__link-text kt-margin-l-5">'
                                                        .__('user::admin-menu.index.TABLE_REMOVE').
                                                    '</span>'.
                                               '</a>'
                                           ]])
                                    </div>
            ...
            @else
                <tr>
                    <td colspan="5" class="text-center">@lang('user::admin-menu.index.DATA_NULL')</td>
                </tr>
            @endif
            </tbody>
        </table>
