...
@section('content')
    ...
    <div id="form-adds">
        <div class="kt-subheader   kt-grid__item" id="kt_subheader">
            ...
            <div class="kt-subheader__toolbar">
                ...
                <div class="btn-group">
                    {{--<button type="button" class="btn btn-primary" onclick="userGroupDefine.save()">--}}
                        {{--@lang('user::user-group-notification.edit.SAVE')--}}
                    {{--</button>--}}
                    <a href="{{route('user.user-group-notification')}}" class="btn btn-secondary">
                        @lang('user::user-group-notification.edit.CANCEL')
                    </a>
                </div>
            </div>
        </div>
                                                            ...
                                                            <div class="kt-widget14__legends ss--padding-left-0">
                                                                <div class="kt-widget14__legend">
                                                                    <span class="kt-widget14__stats kt-portlet__head-title" style="color: #656569;">
                                                                        {{count($user)}} @lang('user::user-group-notification.create.USER_INVALID')</span>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
    ...
    <input type="hidden" value="{{$id}}" id="user_group_id">
@endsection
@section('after_script')
    <script>
        $('.a-add-user').parents('tr').remove();
    </script>
    <script src="{{asset('static/backend/js/user/user-group/edit-user-group-define.js?v='.time())}}" type="text/javascript"></script>
    <script>
        $(document).ajaxStop(function () {
            $('.btn-remove-phone-tr').remove();
            $('.a-add-user').parents('tr').remove();
        });
    </script>
@stop
