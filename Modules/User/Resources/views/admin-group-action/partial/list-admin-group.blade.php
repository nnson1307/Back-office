<table class="table table-striped" id="table-admin-group">
    ...
    @if (isset($list_group) && count($list_group) > 0)
        @foreach ($list_group as $item)
            <tr class="group-item">
                ...
                <td>
                    <select class="form-control" onchange="adminGroupAction.removeRow(this)">
                        <option value="">Hành động</option>
                        <option value="1">@lang('user::admin-group-action.index.TABLE_REMOVE')</option>
                    </select>
                </td>
            </tr>
        @endforeach
    ...
    <tr id="row-btn-add">
        <td colspan="3">
            <a href="javascript:void(0)" onclick="adminGroupAction.showPopupAdminGroup()">
