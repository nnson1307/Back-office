...
@section('content')
    @if (isset($detail))
        <form id="form-submit" method="POST" action="{{ route('user.admin-group-action.update') }}">
            {{ csrf_field() }}
            <input type="hidden" name="action_id" id="action_id" value="{{ $detail['action_id'] }}">
            <div class="kt-subheader kt-grid__item" id="kt_subheader">
                <div class="kt-subheader__main">
                    <h3 class="kt-subheader__title">
                        @lang('user::admin-group-action.index.EDIT_ADMIN_GROUP_ACTION')
                    </h3>
                ...
                <div class="kt-subheader__toolbar">
                    <button type="button" class="btn btn-info btn-bold"
                            id="btn-save-info"
                            onclick="adminGroupAction.saveInfo()">
                        @lang('user::admin-group-action.input.BUTTON_SAVE')
                    </button>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-9 col-xl-9">
                            <input class="form-control{{ $errors->has('action_name') ? ' is-invalid' : '' }}"
                                   ...
                                   placeholder="@lang('user::admin-group-action.input.PLACEHOLDER_NAME')" required>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-9 col-xl-9">
                            <textarea class="form-control" name="description"
                                      id="description" rows="1">{{ old('description', isset($detail['description']) ? $detail['description'] : null) }}</textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-xl-3 col-lg-3 col-form-label">
            ...
            <div class="kt-portlet kt-portlet--tabs">
                <div class="kt-portlet__body">
                    <div class="kt-section">
                        @include('user::admin-group-action.partial.list-admin-group')
                    </div>
                </div>
            </div>
        </form>
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">
                                @lang('user::admin-group-action.edit.ADD_ADMIN_GROUP')
                            </h5>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">
                                @lang('user::admin-group-action.input.BUTTON_CLOSE')
                            </button>
                            <button type="button"
                                    id="btn-add-group-child-to-list"
                                    class="btn btn-primary"
                                    onclick="adminGroupAction.addGroupFromPopup()"
                            >
                                @lang('user::admin-group-action.input.BUTTON_ADD')
                            </button>
                        </div>
                    </div>
                </div>
