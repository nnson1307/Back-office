...
namespace Modules\User\Repositories\AdminGroup;

use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Modules\User\Models\AdminActionTable;
...
use Modules\User\Models\AdminGroupActionTable;
...
class AdminGroupRepository implements AdminGroupRepositoryInterface
{
    ...
    public function __construct(
        ...
        AdminGroupMapTable $adminGroupMap,
    ...
    public function getListNew(array $filters = [])
    {
        ...
        $listAdminGroup = $this->adminGroup->getListNew($filters);
    ...
    public function add(array $data)
    {
        try {
            ...
            return [
                'group_id' => $group_id,
                'error' => 0,
                'message' => __('user::admin-group.create.SUCCESS'),
            ];
        } catch (\Exception $e) {
            DB::rollBack();
            return [
                'error' => 1,
                'message' => __('user::admin-group.create.FAILED'),
            ];
        }
    }
    ...
    public function edit(array $data, $group_id)
    {
        try {
            ...
            return [
                'group_id' => $group_id,
                'error' => 0,
                'message' => __('user::admin-group.edit.SUCCESS'),
            ];
        } catch (\Exception $e) {
            DB::rollBack();
            return [
                'group_id' => $group_id,
                'error' => 1,
                'message' => __('user::admin-group.edit.FAILED'),
            ];
        }
    }
     ...
     * @return void
     */
    private function removeGroupChild($group_parent_id)
    {
        $this->adminGroupMap->remove([
            ['group_parent_id', '=', $group_parent_id],
        ]);
    }

    /**
     * Thêm quan hệ nhóm quyền cha con
     *
     * @param array $data
     * @param int $group_parent_id
     * @return void
     */
    private function addGroupChild(array $data, $group_parent_id)
    {
        $dataGroupChild = [];
        if (isset($data['group_child_id'])) {
            foreach ($data['group_child_id'] as $group_child_id) {
                $dataGroupChild[] = [
                    'group_parent_id' => $group_parent_id,
            ...
            $this->adminGroupMap->addMultipleRecords($dataGroupChild);
        }
    }
