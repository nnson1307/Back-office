...
class AdminMenuCategoryRepository implements AdminMenuCategoryRepositoryInterface
{
    ...
    public function getListNew(array $filters = [])
    {
        ...
        $data = $this->adminMenuCategory->getListNew($filters);
    ...
    public function add(array $data)
    {
        try {
            ...
            return [
                'error' => 0,
                'message' => __('user::admin-menu-category.popup.CREATED'),
                'menu_category_id' => $menu_category_id
            ];
        } catch (\Exception $e) {
            return [
                'error' => 1,
                'message' => __('user::admin-menu-category.popup.CREATE_FAILED'),
            ];
        }
    }
    /**
    ...
    public function edit(array $data)
    {
        try {
            ...
            if ($result) {
                return [
                    'error' => 0,
                    'message' => __('user::admin-menu-category.popup.UPDATED')
                ];
            } else {
                return [
                    'error' => 1,
                    'message' => __('user::admin-menu-category.popup.UPDATE_FAILED')
                ];
            }
        } catch (\Exception $e) {
            return [
